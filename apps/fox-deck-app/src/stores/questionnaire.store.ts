...
import {ref} from "vue";
import type {Question, QuestionMetrics, Questions} from "@/types/question.types";
import axios from "axios";
import {QuestionCompetence} from "@/enum";
import {useQuestionnaireService} from "@/composables/useQuestionnaireService";
import {useQuestionService} from "@/composables/useQuestionService";
...
export const useQuestionnaireStore = defineStore("questionnaireStore", () => {
     ...
     */
    const nextQuestion = async (competence: QuestionCompetence, question: Question,): Promise<void> => {
        ...
        console.debug("[QuestionnaireStore] (nextQuestion) => update question in database");
        ...
        console.debug("[QuestionnaireStore] (nextQuestion) => remove first question from questions");
        questions.value.pop();
    }
    ...
    const setQuestions = (q: Questions): void => {
        console.debug(`[QuestionnaireStore] (updateQuestions) => update questions in state`);
    ...
    const calculateMetrics = (): QuestionMetrics => {
        ...
        return initialQuestions.value.map((question) => question.metrics)
