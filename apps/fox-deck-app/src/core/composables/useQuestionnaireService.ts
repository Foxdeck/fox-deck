...
export function useQuestionnaireService() {
  ...
  const fetchQuestionnaires = async (): Promise<Questionnaires> => {
    console.debug(
      "[QuestionnaireService] (fetchQuestionnaires) => fetch questionnaire from backend.",
    );
    ...
    if (questionnaireResponse.status !== 200) {
      console.debug(
        `[QuestionnaireService] (fetchQuestionnaireById) => error while fetch questionnaire: ${questionnaireResponse.statusText}`,
      );
  ...
  const fetchQuestionnaireById = async (id: string): Promise<Questionnaire> => {
    console.debug(
      `[QuestionnaireService] (fetchQuestionnaireById) => fetch questionnaire with id '${id}' from backend.`,
    );
    const questionnaireResponse = await axios(
      `http://localhost:3000/questionnaires/${id}`,
    );
    if (questionnaireResponse.status !== 200) {
      console.debug(
        `[QuestionnaireService] (fetchQuestionnaireById) => error while fetch questionnaire: ${questionnaireResponse.statusText}`,
